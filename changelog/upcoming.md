...
- 2026-05-13: Ported low-level patterns from `algoscribe` repo. Migrated fetch wrappers to `ApiClient` with exponential backoff and Web/Native `FormData` handling. Replaced static tokens with robust `tokenStore` (supporting `sessionStorage` on Web and `SecureStore` on Native). Added automatic Token Refresh queuing and logging.
- 2026-05-13: UI Refactoring based on `algohealth-consumer`. Introduced `constants/theme.ts` with comprehensive design tokens and `#055A66` as the primary color. Built and integrated standardized `Typography`, `Button`, and `TextInput` UI components across the login and process-voice screens. Removed unused Expo template boilerplate components.
